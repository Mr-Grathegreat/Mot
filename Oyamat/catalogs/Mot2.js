...
global.utils = require("./Mot4.js");
global.loading = require("./Mot3.js");
...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/Mot.json");
...
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../Mot.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('Mot')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('Mot')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('Mot')} file`);
} catch (e) {
  return logger.loader(`can't deploy ${chalk.blueBright('Mot')} file`, "error")
}
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('Mot.json')} file`);
    process.exit(0);
  }
  if (!global.config.PREFIX) {
    logger.error(`please enter your bot prefix in ${chalk.blueBright('Mot.json')} file`)
  }
  if (global.config.author != "Mot") {
    logger.error(`detected : author was changed at ${chalk.blueBright('Mot.json')}`);
    process.exit(0);
  }
  if (packages.author != "Mot") {
    logger.error(`detected : author was changed at ${chalk.blueBright('package.json')}`);
    process.exit(0);
  }
  if (packages.name != "Mot") {
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../Motstate.json"));
  ...
  logger.loader(`deployed ${chalk.blueBright('Motstate')} file`)
} catch (e) {
  return logger.error(`can't read ${chalk.blueBright('Motstate')} file`)
}
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            const configures = require(`../../Mot.json`);
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/Mot.json');
    ...
    global.custom = require('../../Mot.js')({ api: api });
...
(async () => {
  try {
    ...
    logger(`deployed ${chalk.blueBright('database')} system`, "Mot");
    const botData = {};
    botData.models = models;
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "Mot") }
})();